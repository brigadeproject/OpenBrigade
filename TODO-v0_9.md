...
- Add outbound reply support.
  ...
  Status: implemented for live webhooks through Bot API `sendMessage`.
- Add auth and allowlist controls.
  Acceptance: unknown Telegram users are blocked or routed to owner approval by default.
  Status: implemented with disabled-by-default webhooks, shared webhook secret, allowlist, and owner approval records.
- Add tests with fake Telegram payloads.
  Acceptance: no live Telegram network is required for unit tests.
  Status: implemented.
  Remaining: live provider smoke gated by explicit operator env.
...
- Add outbound reply support.
  ...
  Status: implemented for live webhooks through synchronous Google Chat reply payloads.
- Add user identity mapping.
  Acceptance: Google identities map to OpenBrigade users or pending approval records.
  Status: implemented with durable pending/approved/rejected identity records.
...
- Add OAuth/API-key documentation for the supported first pass.
  ...
  Status: implemented with API-key envs plus local OAuth credential import/status/logout.
...
- Add failure behavior for missing/invalid credentials.
  ...
  Status: implemented for provider construction and CLI/model-call surfaces.
...
- Add connector rate limits and message size limits.
  ...
  Status: implemented for live webhooks with Redis runtime rate limiting.
- Add connector audit records.
  ...
  Status: implemented with durable connector audit records and redacted metadata.
- Add connector disable switches.
  Acceptance: each external connection can be disabled without editing code.
  Status: implemented with per-connector webhook enable flags.
...
- Threat-model zero users.
  ...
  Status: implemented with ASGI coverage and explicit bootstrap browser state; write controls remain
  permission-gated.
- Threat-model one user and one owner.
  ...
  Status: implemented with fail-closed auth-required behavior plus browser messaging for implicit
...
- Threat-model `require_auth=false` on a reachable host.
  ...
  Status: implemented through backend warning/audit plus cockpit/status warning banners.
- Threat-model stale, expired, malformed, and CR/LF-bearing bearer tokens.
  Acceptance: all fail cleanly with no route execution.
  Status: implemented with backend malformed/expired token coverage and React-side expired/malformed
  token handling.
...
- Replace route-registration-only web tests with running-service tests.
  ...
  Status: implemented with direct ASGI harness plus auth-aware browser smoke support.
- Cover `/api/auth/me`.
...
- Cover `/api/teams` and denied writes.
  ...
  Status: implemented for denied writes and role-aware browser behavior.
...
- Add token expiry handling in the React UI.
  ...
  Status: implemented with local JWT expiry detection, paused SSE reconnect, and visible status.
- Add role-aware UI behavior.
  Acceptance: observer/operator/owner see appropriate controls and denied actions are clear.
  Status: implemented with permission-gated controls and visible missing-permission notices.
- Add web smoke with auth enabled.
  Acceptance: full web workflow passes with `BRIGADE_REQUIRE_AUTH=true`.
  Status: implemented in `ops/web-browser-smoke.sh`; set `BRIGADE_TOKEN` to seed the browser session
  through Chrome DevTools for auth-enabled smoke. Clean-stack auth-required execution remains a
  v0.9.3 release-validation gate.
...
- Make task workflows useful.
  ...
  Status: implemented for creation, filtering, agent follow-through, and blocker visibility.
- Make chat workflows useful.
  ...
  Status: implemented for selected-agent and orchestrator chat with model selection and persisted messages.
- Make team/hierarchy workflows useful.
  ...
  Status: implemented for current team visibility and permitted delegation-policy edits.
- Make settings/status safe and clear.
  ...
  Status: implemented with redacted settings payload, config hash display, datastore status, and auth/bind warnings.
- Add responsive layout and browser tests.
  ...
  Status: implemented with Chromium desktop/mobile smoke screenshots in `ops/web-browser-smoke.sh`.
- Add default and per-agent model selection to the cockpit view.
  Acceptance: Agent widget in cockpit view has a dropdown of actively available models.
  Acceptance: Orchestrator chat widget in cockpit view has a dropdown of actively available models.
  Explicitly Not: Agent sidebar in Ops Room View does not have a dropdown.
  Status: implemented with `/api/models`, agent model selections, and orchestrator model selection.
...
- Send a web chat and verify persisted side effects.
  ...
  Status: implemented in ASGI tests; live Ollama chat smoke passed through web API.
- Recreate containers without volume loss.
  Acceptance: web, auth, messages, and hierarchy state remain intact.
  Status: non-dropping recovery passed; auth-enabled browser flow is covered by the smoke script and
  remains a v0.9.3 clean-stack validation gate.

### TTY/TUI chat interface

- Command line executable that allows the user to enter into a full page chat with a single agent, using their primary chat channel.
- Acceptance: User can connect to the gateway and chat, switching agents with slash commands.
  Status: implemented as `./ops/brigade-live.sh chat tui --agent <id>` with `/agent`, `/agents`, `/refresh`, and `/quit`.

### Web/GUI Chat Interface

- Connect web chats to models
  Acceptance: Chats from GUI send and receive messages.
  Status: implemented for selected-agent and orchestrator chat; live Ollama web API smokes passed.
