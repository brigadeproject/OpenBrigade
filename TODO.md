...
Current RC execution checklist: `RC_PUNCHLIST_v1_0.md`.
...
- Replace remaining prototype app-data runtime state with the intended datastore-backed persistence layer. `Complete for operator workflows: Postgres-backed runtime state is required, with Redis/Qdrant/Neo4j adapters active in the release stack.`
...
- Runtime data now lives in the `brigade_` Postgres/Redis/Qdrant/Neo4j stack. Wipe/reseed is a
  deliberate recovery operation and must be preceded by an explicit backup when data matters.
...
- Backend web auth/API hardening. `Implemented: fail-closed auth, malformed/expired token coverage, denied writes, settings redaction, security headers, Docker asset packaging, ASGI API tests, and token-seeded browser smoke support. Clean-stack auth-required execution remains a v0.9.3 validation gate.`
...
- [x] Telegram inbound fake-payload wrapper, allowlist, and tests.
- [x] Google Chat inbound fake-event wrapper, allowlist metadata, and tests.
- [x] OpenAI/Codex LiteLLM alias, redacted settings, API-key/OAuth credential status.
- [x] Gemini LiteLLM alias, redacted settings, API-key/OAuth credential status.
- [x] Connector disable switches, rate limits, message size limits, and approval/audit surfaces.
- [x] Setup, bounded-smoke, disable, outbound behavior, and rollback docs: `CONNECTORS_RUNBOOK.md`.
- [ ] Execute bounded live smoke with real Telegram/Google Chat/OpenAI/Gemini credentials when an
  operator intentionally provides those credentials.
...
- Browser quality:
  ...
  - Auth-enabled browser smoke is wired through `BRIGADE_TOKEN`; final clean-stack execution remains part of v0.9.3 validation.
